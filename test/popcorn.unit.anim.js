...
//  console.log( $static );
  ...
  Popcorn.forEach( tests, function( test ) {
    equals( Popcorn( "#vid-" + test.id , { frameAnimation: true }).position().top,  test.top,  "Popcorn('#vid-" + test.id + "', { frameAnimation: true }).position().top" );
    equals( Popcorn( "#vid-" + test.id, { frameAnimation: true } ).position().left, test.left, "Popcorn('#vid-" + test.id + "', { frameAnimation: true }).position().left" );
  });
  ...
  try {
    ok( Popcorn( "#audio", { frameAnimation: true } ).position(), "position called from audio" );
  } catch( e ) {
    ok( false, e );
  }
...
test("Remove Plugin", function () {
  ...
  var p = Popcorn("#video", { frameAnimation: true }),
      p2 = Popcorn("#video", { frameAnimation: true }),
...
test("Defaulting Empty End Values", function() {
  ...
  var popped = Popcorn( document.createElement( "audio" ), { frameAnimation: true } )
  .play()
  .testdefault({
    start: 0, // seconds
  ...
  var popped2 = Popcorn( document.createElement( "video" ), { frameAnimation: true } )
  .play()
  .testdefault({
    start: 0, // seconds
    apikey: "CHAyhB5IisvLqqzGYNYbmA",
...
test("Functions", function () {
  ...
  var popped = Popcorn("#video", { frameAnimation: true }), ffTrackId, rwTrackId, rw2TrackId, rw3TrackId, historyRef, trackEvents;
...
test("Frame Animation", function () {
  ...
  Popcorn.plugin("frame", {

    start: function () {

      started = true;
      ok(!frameCount, "start runs before frame (@ " + popped.currentTime() + ")");
      plus();
    ...
    frame: function () {
      ...
      if (!frameCount) {
        ok(!ended, "frame runs before end (@ " + popped.currentTime() + ")");
        plus();
      }
      ...
      //frame should run no more than 125 times in 2 seconds
      ok(frameCount, "frame has run at least once (" + frameCount + " time" + (frameCount !== 1 ? "s" : "") + ")");
      plus();

      ok(started, "end runs after start (@ " + popped.currentTime() + ")");
      plus();
...
test("Parsing Functions", function () {
  ...
  var expects = 3,
      count = 0,
      popperly = Popcorn("#video", { frameAnimation: true });
...
test("Parsing Integrity", function () {
  ...
  var expects = 6,
      count = 0,
      timeOut = 0,
      poppercore = Popcorn( "#video", { frameAnimation: true } );
...
test("Parser Support", function () {
  ...
  var expects = 3,
      ...
      timeOut = 0,
      interval,
      audiocorn = Popcorn( "#audio", { frameAnimation: true } );
